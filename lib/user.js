...
exports.list = (cb) => {
  ...
  for(var u in db){
    users.push({name: db[u].name, pass: db[u].pass, uid: db[u].uid, admin: db[u].admin});
  }
  cb(undefined, users);
};
...
exports.add = (u, cb) => {
  ...
  //
  if(u.name in db){
    cb('Username is in use, could not add user.')
  }
  else{
    var temp = (u.admin == 'yes' || u.admin == 'true') ? true : false;
    var newUser = user(u.name, u.pass, temp);
    u.uid = newUser.uid;
    db[u.name] = newUser;
    cb(undefined, u)
  }
